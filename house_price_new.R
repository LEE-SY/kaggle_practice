...
kc_house<-read.csv("train.csv", na.strings = "NA",stringsAsFactors = FALSE) #data loading
...
write.csv(kc_house,"kc_house_pre.csv",row.names = FALSE)
...
ScatterHist(kc_house, "sqft_lot15_log", "price_log",
            smoothmethod="lm",
            estimate_sig = TRUE, 
            minimal_labels = TRUE, 
            binwidth_x = NULL,
...
full_m<-lm(price_log~.^2, data=kc_house_test)

summary(full_m)

null_m<-lm(price_log~1^2.,data=kc_house_test)
